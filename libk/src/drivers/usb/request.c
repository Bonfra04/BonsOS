...
{
    usb_request_packet_t* setup = pfa_alloc(1);
...
{
    usb_request_packet_t* setup = pfa_alloc(1);
    setup->type = USB_REQUEST_DIR_DEVICE_TO_HOST | USB_REQUEST_TYPE_STANDARD | USB_REQUEST_RECP_DEVICE;
    setup->request = USB_REQUEST_GET_DESCRIPTOR;
    setup->value = (desc_type << 8) | desc_index;
    setup->index = 0;
    setup->size = size;

    return usb_transfer_control_in(dev->bus, dev->addr, 0, setup, buffer, size);
}
