...
static bool send_scsi_command(void* device, scsi_command_t* command, void* data, size_t transfer_length)
{
    ...
    {
        usb_transfer_status_t x = usb_transfer_bulk_in(dev->device->bus, dev->device->addr, ep->endpoint_number, data, transfer_length);
        kernel_log("B %d\n", (int)x);
    }
   
    kernel_log("Got here 3\n");
...
static void init_device(usb_device_t* device)
{
    ...
    kernel_log("Found %hhd luns", lun_count);
