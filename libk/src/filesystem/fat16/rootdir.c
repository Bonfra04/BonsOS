...
file_t open_entry(fs_data_t* fs, const char* filename, const char* mode, bool is_file)
{
    ...
    data->dir_entry_address = mount_info->root_offset + sizeof(dir_entry_t) * entry_index;
    ...
    if(*mode == 'a')
    {
        ...
        uint16_t next_clustet;
        get_next_cluster(fs, &next_clustet, data->cluster);
    ...
    file.flags = is_file ? FS_FILE : FS_DIRECTORY;
...
{
    return open_entry(fs, filename, "r", false);
}
...
bool rootdir_create_dir(fs_data_t* fs, const char* dirpath)
{
    ...
    file_t dir = rootdir_open_dir(fs, dirpath);
